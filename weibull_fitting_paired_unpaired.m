...
sheets = {'RB_analysis', 'AP_analysis', 'AP'};
...
for sheet_idx = 1:length(sheets)
    ...
    x_values = data(:, 1); % Assuming the first column is the x-values (stimulus pairing range)
    figure;
    % Loop through each eccentricity (3, 4, 5, 6, 7 deg)
    for i = 2:6
        y_values = data(:, i) / 100; % Normalize the y-values
        ...
        plot(x_values, data(:, i), 'o', 'MarkerFaceColor', colors{i-1}, 'Color', colors{i-1}); % Original data points
        hold on;
        plot(x_fit, y_fit * 100, '--', 'Color', colors{i-1}); % Fitted curve (scaled back to percentages)
...
%         thresholds(i-1, sheet_idx) = lambda * (log(2))^(1/k);
        ...
        thresholds(i-1, sheet_idx) = lambda * (log(1/(1-0.75)))^(1/k);
