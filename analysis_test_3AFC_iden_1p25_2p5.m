...
trialsdir = pname;
cd(trialsdir);
[d dTname lenTname] = getmdata_xh_ch12_13_14(trialsdir, 'i,');
...
x2_single_double     = [1:7,29];
x2_double_single     = [8:14,30];
x2_Nsingle_Ndouble     = [15:21,31];
x2_Ndouble_Nsingle     = [22:28,32];
...
x_len = linspace(log(1.25),log(2.5),5);
dif = x_len(2)-x_len(1);
x_len = [x_len(1)-dif x_len x_len(5)+dif x_len(5)+dif+dif];


x2faster_rate = x2_resp(:,1)./(x2_resp(:,4)-x2_resp(:,3));
x2slower_rate = x2_resp(:,2)./(x2_resp(:,4)-x2_resp(:,3));
% x2_resp_fa = x2_resp(:,1)./x2_resp(:,4); 
figure; plot(x_len, x2faster_rate,'r-'); hold on; 
%  plot(x_len, x2slower_rate,'b-'); hold on; 
xticks(x_len)
ylabel("fast proportion")

% x4_single_double_accuracy = 
