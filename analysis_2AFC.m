...
trialsdir = pname;
cd(trialsdir);
[d dTname lenTname] = getmdata_xh_ch12_13(trialsdir, 'l,');
...
START_TIME = 600;  % examine respone after 600ms
...
total_cdnum = 40;
first_rise_loc_diff_chan1213=ones(1,total_trial_n);
...
chanOR1213 = chan12 + chan13;

% difference chan6-14, finding the rising phase
diff_chan1213= diff(chanOR1213);
...
%       end    
      ...
      if (max(diff_chan1213(:,i)) > THRESHOLD)
        first_rise_loc_diff_chan1213(i) = find(diff_chan1213(:,i) > THRESHOLD, 1 )+1;
    else
        first_rise_loc_diff_chan1213(i) = -999;
    end
    ...
    if first_rise_loc_diff_chan1213(i) < FIRST_DELAY,
        sprintf('First EVENT OCCURRED TOO SOON');
        Evnt_1(i) = -3;
        Evnt_2(i) = -3;
        missed_trial_ct = missed_trial_ct + 1;
...
%      end   
     ...
     if Evnt_1(i) == 0
        ...
        if     (chan12(first_rise_loc_diff_chan1213(i),i) > THRESHOLD) && ...
             (chan13(first_rise_loc_diff_chan1213(i),i) < BASELINE)
%              (chan14(first_rise_loc_diff_chan121314(i),i) < BASELINE)
            Evnt_1(i) = 12;
        elseif (chan13(first_rise_loc_diff_chan1213(i),i) > THRESHOLD) && ...
             (chan12(first_rise_loc_diff_chan1213(i),i) < BASELINE)
%              (chan14(first_rise_loc_diff_chan121314(i),i) < BASELINE)
            Evnt_1(i) = 13; 
...
rest_together = [Resp12',Resp13'];
rest_together(:,3) = sum(rest_together,2);

%extraction
x4_single_resp = rest_together(x4_single,:)+rest_together(x4_Nsingle,:);
x4_double_resp = rest_together(x4_double,:)+rest_together(x4_Ndouble,:);
% x4_double_single_resp = x4_double_single_resp1(:,[2 1 3 4]); 
% x4_resp = x4_single_resp + x4_double_resp; 

x2_single_resp = rest_together(x2_single,:)+rest_together(x2_Nsingle,:);
x2_double_resp = rest_together(x2_double,:)+rest_together(x2_Ndouble,:);
% x2_double_single_resp = x2_double_single_resp1(:,[2 1 3 4]); 
% x2_resp = x2_single_double_resp + x2_double_single_resp;

%calculation
x4_resp_hits = x4_double_resp(:,2)./x4_double_resp(:,3);
% x4_resp_falseR = x4_resp(:,3)./x4_resp(:,4);
x4_resp_fa = x4_single_resp(:,2)./x4_double_resp(:,3); 

x2_resp_hits = x2_double_resp(:,2)./x2_double_resp(:,3);
% x2_resp_falseR = x2_resp(:,3)./x2_resp(:,4);
x2_resp_fa = x2_single_resp(:,2)./x2_single_resp(:,3); 
