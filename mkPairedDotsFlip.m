...
function stim = mkPairedDotsFlip(stim)
...
if ~isfield(stim, 'contrast');   stim.contrast = [0.89 0.89];    end
if ~isfield(stim, 'dotSz');      stim.dotSz = 2;                 end
if ~isfield(stim, 'luminance');  stim.luminance = [1 1];         end
if ~isfield(stim, 'pathlength'); stim.pathlength = 7;            end
if ~isfield(stim, 'colors');     stim.colors = [1 1 1; 1 1 1];   end
...
lum(1) = 4.45;
if (stim.luminance == -1)
    lum(2) = lum(1).*(stim.contrast(2)+1)./(1-stim.contrast(1));
    lum(3) = lum(1).*(stim.contrast(2)+1)./(1-stim.contrast(2));
    lum = lum./80.1;
else
    lum(2) = stim.luminance(1);
    lum(3) = stim.luminance(2);
end
loLum = 0;%lum(1);
hiLum = lum(2:3);
color1 = reshape(stim.colors(1,:),1,1,3).*hiLum(1);
color2 = reshape(stim.colors(2,:),1,1,3).*hiLum(2);

% Other dots
pathlength = stim.pathlength;         %in degrees
lifetime = pathlength./stim.speed(1); %in frames

dot2dist = ([sin(stim.direction(1)-pi/2),cos(stim.direction(1)-pi/2)]).*sin(stim.ds./2)*pathlength;
...
validPos = pos & ((xFrame-cStim(2)+dot2dist(2)).^2+(yFrame-cStim(1)-dot2dist(1)).^2)<=stim.maskRad^2;
% figure; flipBook(pos-validPos);
pos = find(pos == 1);
validPos = find(validPos == 1);

nDots = max([1, round(stim.density.*length(validPos))]);

nDotsNonCoherent = round((1-stim.coherence).*stim.density.*length(validPos));
...
% dot.
dotInds = randperm(length(validPos), nDots);
dotInds = validPos(dotInds);
...
dot2kill = 1;
dotPositions2 = round(dotPositions + repmat(dot2dist,nDots,1));
...
for t = 1:stim.size(3)
    ...
    dotVelocity = [sin(stim.direction(t)-stim.ds/2), cos(stim.direction(t)-stim.ds/2)];
    dotVelocity2 = [sin(stim.direction(t)+stim.ds/2), cos(stim.direction(t)+stim.ds/2)];
    dotVelocity = dotVelocity*stim.speed(t);
    dotVelocity2 = dotVelocity2*stim.speed(t);
    dotPositions = dotPositions + repmat(dotVelocity, size(dotPositions, 1), 1);
    dotPositions2 = dotPositions2 + repmat(dotVelocity2, size(dotPositions2, 1), 1);
    tmpDotPositions = round(dotPositions);
    tmpDotPositions2 = round(dotPositions2);
    dotInds = sub2ind(frameSz, tmpDotPositions(:,1), tmpDotPositions(:,2));
    
    % FOR RANDOMIZED NON-COHERENCE IB_EDIT
    if nDotsNonCoherent>0
        dotIndsNonCoherent = randperm(length(validPos), nDotsNonCoherent);
        dotIndsNonCoherent = validPos(dotIndsNonCoherent);
        overlap = ismember(dotIndsNonCoherent,dotInds);
        while sum(overlap)>0
            newInds = randperm(length(validPos), sum(overlap));
            dotIndsNonCoherent(overlap) = validPos(newInds);
            overlap = ismember(dotIndsNonCoherent,dotInds);
        end
        ...
        tmpDotPositions2(randDotIndex,:) = round(tmpDotPositions(randDotIndex,:) + repmat(dot2dist,nDotsNonCoherent,1));
        dotPositions(randDotIndex,:) = tmpDotPositions(randDotIndex,:);
        dotPositions2(randDotIndex,:) = tmpDotPositions2(randDotIndex,:);
    end
    
    % FOR LIFETIME RESTRICTION
    if nKill(t)>0
        dotIndsRespawn = randperm(length(validPos), nKill(t));
        dotIndsRespawn = validPos(dotIndsRespawn);
        overlap = ismember(dotIndsRespawn,dotInds);
        while sum(overlap)>0
            newInds = randperm(length(validPos), sum(overlap));
            dotIndsRespawn(overlap) = validPos(newInds);
            overlap = ismember(dotIndsRespawn,dotInds);
        end
        ...
        tmpDotPositions2(kInds,:) = round(tmpDotPositions(kInds,:) + repmat(dot2dist,nKill(t),1));
        dotPositions(kInds,:) = tmpDotPositions(kInds,:);
        dotPositions2(kInds,:) = tmpDotPositions2(kInds,:);
%         dotInds2 = sub2ind(frameSz, tmpDotPositions2(:,1), tmpDotPositions2(:,2));
    end
    ...
    w = ~ismember(dotInds,pos)|~ismember(dotInds2,pos);
    if sum(w)>0
        tmpCStim = repmat(cStim,sum(w),1);
        tmpDots = tmpDotPositions(w,:)-tmpCStim;
        tmpDots2 = tmpDotPositions2(w,:)-tmpCStim;
        d = (tmpDots(:,2)+a*tmpDots(:,1))/(1+a^2);
        d2 = (tmpDots2(:,2)+a*tmpDots(:,1))/(1+a^2);
        tmpDots = [2*d*a-tmpDots(:,1), 2*d-tmpDots(:,2)];
        tmpDots2 = [2*d2*a-tmpDots2(:,1), 2*d2-tmpDots2(:,2)];
        tmpDotPositions(w,:) = round(tmpDots+cStim+repmat(dotVelocity, sum(w), 1));
        tmpDotPositions2(w,:) = round(tmpDots2+cStim+repmat(dotVelocity2, sum(w), 1));
        dotPositions(w,:) = tmpDotPositions(w,:);
        dotPositions2(w,:) = tmpDotPositions2(w,:);
        dotInds2(w,:) = sub2ind(frameSz, tmpDotPositions2(w,1), tmpDotPositions2(w,2));
    end
            ...
            for j = 1:3
                for i = 1:size(tmpDotPositions,1)
                    thisFrame(tmpDotPositions(i,1)-rad:tmpDotPositions(i,1)+rad, ...
                        tmpDotPositions(i,2)-rad:tmpDotPositions(i,2)+rad,j) = color1(j);
                    thisFrame(tmpDotPositions2(i,1)-rad:tmpDotPositions2(i,1)+rad, ...
                        tmpDotPositions2(i,2)-rad:tmpDotPositions2(i,2)+rad,j) = color2(j);
                end
            end
        else
            for j = 1:3
                for i = 1:size(tmpDotPositions,1)
                    thisFrame(tmpDotPositions(i,1)-rad+1:tmpDotPositions(i,1)+rad, ...
                        tmpDotPositions(i,2)-rad+1:tmpDotPositions(i,2)+rad,j) = color1(j);
                    thisFrame(tmpDotPositions2(i,1)-rad+1:tmpDotPositions2(i,1)+rad, ...
                        tmpDotPositions2(i,2)-rad+1:tmpDotPositions2(i,2)+rad,j) = color2(j);
                end
            end
        end
    else
        for j = 1:3
            thisFrame(tmpDotPositions,j) = color1(j);
            thisFrame(tmpDotPositions,j) = color2(j);
        end
    end
%     size(thisFrame)
